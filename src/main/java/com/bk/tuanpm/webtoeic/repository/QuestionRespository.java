...
public interface QuestionRespository extends JpaRepository<Question, Integer> {
    ...
    @Query(value = "SELECT e.id_exam,\n" +
            ...
            "WHERE e.id_exam = ?1 AND p.type = ?2\n" +
            "ORDER BY p.type,\n" +
            "         p.id_part,\n" +
            "         s.id_set,\n" +
            "         q.cauhoibaithithuid;", nativeQuery = true)
    List<Object[]> getListExamQuestionDTO(int examId, String type);
    ...
    @Query(value = "SELECT e.id_exam,\n" +
            ...
            "INNER JOIN dbo.part p ON s.part_toeic_id_part = p.id_part\n" +
            "INNER JOIN dbo.question q ON s.id_set = q.id_set\n" +
            "INNER JOIN dbo.explain_detail ed ON q.cauhoibaithithuid = ed.question_cauhoibaithithuid\n" +
            "WHERE e.id_exam = ?1\n" +
            "ORDER BY p.type,\n" +
            "         p.id_part,\n" +
            "         s.id_set,\n" +
            "         q.cauhoibaithithuid;", nativeQuery = true)
    List<Object[]> getAllListExamQuestionDTO(int examId);
